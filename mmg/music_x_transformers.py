...
class MusicTransformerWrapper(nn.Module):
    def __init__(
        ...
        if self.prompt_enable:
            ...
            self.token_emb_prompt = nn.Embedding.from_pretrained(
                LLAMA_checkpoint['tok_embeddings.weight'][:, :emb_dim])
            self.token_emb_prompt.weight.requires_grad = prompt_enable
            self.token_emb_prompt = TokenEmbedding(emb_dim, 32000, l2norm_embed=l2norm_embed)
        ...
        if self.prompt_enable:
            self.to_logits_prompt = nn.Linear(dim, 32000)
    ...
    def forward(
        ...
        b, seqlen, featurelen = x.shape
        num_mem = self.num_memory_tokens
        # match the data dimension

        x = sum(
            emb(x[..., i]) for i, emb in enumerate(self.token_emb)
        ) + self.pos_emb(x)
        if self.prompt_enable:
            x = x + self.token_emb_prompt(prompt)
        ...
        out_prompt = None
        if self.prompt_enable:
            out_prompt = self.to_logits_prompt(x)
        ...
        if return_attn:
            ...
            return out, out_prompt, attn_maps
...
class MusicAutoregressiveWrapper(nn.Module):
    ...
    def generate(
        ...
        for _ in range(seq_len):
            ...
            # cut the prompt
            prompt = prompt[:, :out.shape[1]]

            if return_attn:
                logits, attn = self.net(
                    x, prompt, mask=mask, return_attn=True, **kwargs
                )
                logits = [l[:, -1, :] for l in logits]
            else:
                logits = [
                    l[:, -1, :] for l in self.net(x, prompt, mask=mask, **kwargs)[0]
                ]
    ...
    def forward(self, x, prompt, attribute, return_list=False, **kwargs):
        xi = x[:, :-1]
        xo = x[:, 1:]
        ...
        out, out_prompt = self.net(xi, prompt[:, :-1], **kwargs)
        losses = [
            F.cross_entropy(
                out[i].transpose(1, 2),
                xo[..., i],
...
class MusicXTransformer(nn.Module):
    ...
    def forward(self, seq, prompt, attribute, mask=None, **kwargs):
        return self.decoder(seq, prompt, attribute, mask=mask, **kwargs)
